...
#if defined(NDEBUG)
# error "Subatomchain cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Subatomchain Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Subatomchain: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Subatomchain: find block wanted by given orphan block
...
// Subatomchain: find last block index up to pindex
...
{
    //powstodo
    if(!fProofOfStake && pindexLast->nHeight < 10000)
    {
    ...
    int64_t nInterval = nTargetTimespan / nTargetSpacing; //16*60 / 60 = 16
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Subatomchain miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Subatomchain: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Subatomchain: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Subatomchain: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Subatomchain: track money supply and mint amount info
...
// Subatomchain: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Subatomchain: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Subatomchain: compute stake entropy bit for stake modifier
    ...
    // Subatomchain: compute stake modifier
...
uint256 CBlock::GetHash() const
{
    ...
    case 2:
        return GetPoWHash();
    case 1:
        return SCHash(BEGIN(nVersion), END(nNonce));
    default:
        return SCHash(BEGIN(nVersion), END(nNonce));
    }
}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Subatomchain: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Subatomchain: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Subatomchain: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Subatomchain-loadblk");
