...
Value setmint(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            "setmint <generate>\n"
            "<generate> is true or false to turn POS generation on or off.");

    bool fGenerate = true;
    if (params.size() > 0)
        fGenerate = params[0].get_bool();

   if(fGenerate) fStopPos = false;
   else fStopPos = true;
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Subatomchain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Subatomchain is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Subatomchain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Subatomchain is downloading blocks...");
