...
static const int64_t MAX_MONEY = 84000000 * COIN; //newtodo1
...
inline unsigned int GetTargetSpacing(int nHeight) { return /*IsProtocolV2(nHeight) ? 64 :*/ 120; }
...
public:
    ...
    {
        // Subatomchain: the coin stake transaction is marked with the first output empty
...
public:
    ...
    // Subatomchain: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Subatomchain: two types of block: proof-of-work or proof-of-stake
    ...
    // Subatomchain: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Subatomchain: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Subatomchain: block index flags
    enum  
    {
